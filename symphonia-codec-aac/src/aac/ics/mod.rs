...
const NORMAL_SCALE_MIN: i16 = -100;
    ...
    static POW43_TABLE: LazyLock<Box<[f32; 8192]>> =  LazyLock::new(|| {
    ...
    static NORMAL_SCF_TABLE: LazyLock<Box<[f32; 256]>> = LazyLock::new(|| {
    ...
    static INTENSITY_SCF_TABLE: LazyLock<Box<[f32; 256]>> = LazyLock::new(|| {
        let mut table = [0f32; 256];
        for (i, table) in table.iter_mut().enumerate() {
